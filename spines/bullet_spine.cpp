...
class CommandLineArguments {
 public:
  ...
  explicit CommandLineArguments(const std::vector<std::string>& args) {
    for (size_t i = 0; i < args.size(); i++) {
      ...
      if (arg == "-h" || arg == "--help") {
        ...
        log_path = args.at(++i);
        spdlog::info("Command line: log_path = {}", log_path);
  ...
  inline void print_usage(const char* name) noexcept {
    ...
    std::cout
        << "--log-dir <path>\n"
        << "    Write log file to this directory with a generic filename.\n";
    std::cout << "--log-path <path>\n"
              << "    Path to the output log file (overrides --log-dir).\n";
...
int main(const char* argv0, const CommandLineArguments& args) {
  ...
  observation.append_observer(measurement_model);

  // Observation: Contact filter
  auto contact_filter = std::make_shared<ContactFilter>(/* p_contact = */ 0.5);
  observation.append_observer(contact_filter);
  ...
  bullet_params.position_base_in_world =
      Eigen::Vector3d(0., 0., args.base_altitude);
